...
import { googleAuthService } from '../services/googleAuthNew.js';
...
export class AuthModal {
  ...
  private createGoogleButton(): HTMLElement {
    ...
    // Crea un div per il bottone Google che verrà popolato da Google Identity Services
    ...
    // Inizializza il bottone Google dopo un breve delay per assicurarsi che l'elemento sia nel DOM
    setTimeout(() => {
      try {
        ...
        googleButtonDiv.appendChild(fallbackButton);
      }
    }, 100);
  ...
  private async handleGoogleLogin(): Promise<void> {
    try {
      // Questo metodo sarà chiamato solo se il bottone automatico non funziona
